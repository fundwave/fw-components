  ...
  `; }
    ...
    createFontPickComponent(font) {
      ...
      @value-changed=${(e) => {
            ...
            this.shadowRoot?.dispatchEvent(new CustomEvent("font-changed", {
                detail,
                bubbles: true,
                composed: true
            }));
    ...
    createSizePickComponent(size) {
      ...
      @value-changed=${(e) => {
            ...
            this.shadowRoot?.dispatchEvent(new CustomEvent("size-changed", {
                detail,
                bubbles: true,
                composed: true
            }));
        ...
        switch (this.nav) {
          ...
          ${(Object.keys(this.theme) ?? []).map((section) => html ` <button part="theme-button" @click=${() => this.sectionChangeHandler(`${section}`)}>${section}</button>`)}
            ...
            case ThemeEnum.COLORS:
                content = html ` <div part="content-container">
          ${(Object.keys(this.theme.colors) ?? []).map((color) => html ` <button part="theme-button" @click=${() => this.sectionChangeHandler(`colors-${color}`)}>${msg(color)}</button>`)}
        </div>`;
                break;
          ...
          ${this.theme[ThemeEnum.SIZES] && Object.keys(this.theme[ThemeEnum.SIZES]).length !== 0
            ...
            case ThemeEnum.FONTS:
                content = html ` <span part="content-container"> ${Object.keys(this.theme[ThemeEnum.FONTS]).map((font) => this.createFontPickComponent(font))} </span>`;
                break;
            default:
                content = html ` <span part="content-container">
