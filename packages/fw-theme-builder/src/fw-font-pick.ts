...
export class FwFontPick extends LitElement {
  ...
  async optionSelectHandler(e: CustomEvent) {
    ...
    (document.fonts as FontFaceSet).add(selectedFont);
    ...
    return html`
      ...
      <span part="font-container">
        ...
        <div part="font-button">
          <paper-dropdown-menu part="font-dropdown-container" .dynamicAlign=${true} class="plain" no-label-float @iron-select=${(e: CustomEvent) => this.optionSelectHandler(e)} style="width:100%">
            <paper-listbox part="font-listbox" style="color:#000; max-height:200px; overflow-y:auto;" slot="dropdown-content" .selected=${this.value.name || ""} attr-for-selected="name">
              ${this.options && repeat(this.options, (item) => html`<paper-item part="font-dropdown-option" style="font-family: ${item.style}" .name=${item.name} .obj=${item}>${item.name}</paper-item>`)}
            </paper-listbox>
          </paper-dropdown-menu>
        </div>
      </span>
